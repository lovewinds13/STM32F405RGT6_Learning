...
void spi_gpio_init(uint8_t spi_chl)
{
	...
	if (spi_chl == 1)
	{
		RCC_AHB1PeriphClockCmd(RCC_PCLK_SPIM1_GPIO, ENABLE);		//开启SPIM1 GPIO时钟
		
		//SP1对应引脚复用映射
		GPIO_PinAFConfig(SPIM1_GPIO_PORT, GPIO_PinSource5, GPIO_AF_SPI1); //GPIOA5复用为SPI1
		GPIO_PinAFConfig(SPIM1_GPIO_PORT, GPIO_PinSource6, GPIO_AF_SPI1); //GPIOA6复用为SPI1
		GPIO_PinAFConfig(SPIM1_GPIO_PORT, GPIO_PinSource7, GPIO_AF_SPI1); //GPIOA7复用为SPI1
